...
> **⚠️ MSS_L2 is nearly full (~5 KB free, fragmented).** The R5F image lives in `MSS_L2` (0xB0000). Adding code/data to MSS files often fails linking not with "out of memory" but with a *fragmentation* error:
> `run placement with alignment fails for section "GROUP_6"` (the R5F mode-stack group in `ti/platform/awr2x44P/r5f_linker.cmd`) — note `unused` may exceed the section size while `max hole` is smaller than it.
> **Fix:** relocate the new module out of MSS_L2 in `mss/mmw_mss_linker.cmd`. `TCMA_RAM` (0x100, ~64 KB, **executable**) and `TCMB_RAM` (0x80000, ~64 KB, RW) are nearly empty. Place the object's code/rodata in TCMA and its data/bss in TCMB, scoped by object file — the TI linker routes by *most-specific match*, so an object-scoped spec wins over the platform's generic `.text:` / `.bss:` catch-alls even though the platform `.cmd` is listed first:
> ```
> .myModCode: { mymod.oer5f(.text) mymod.oer5f(.text.*) mymod.oer5f(.rodata) mymod.oer5f(.rodata.*) } > TCMA_RAM
> .myModData: { mymod.oer5f(.data) mymod.oer5f(.data.*) mymod.oer5f(.bss)  mymod.oer5f(.bss.*) }  > TCMB_RAM
> ```
> Code placed in TCMA is >32 MB from MSS_L2 callers, so the linker auto-generates small trampolines/veneers in MSS_L2 (~8 B each) — harmless. Use `.bss.<name> (NOLOAD)` for state that must survive a warm reset (e.g. a crash black-box). The existing `.ota_text > TCMA_RAM` / `.ota_data > TCMB_RAM` lines are the precedent for this split. See `mss/wf_debug_info.c` for a worked example.
